...
- **Closed trace event taxonomy** (issue #115): versioned `EventType` and `ActorType` enums in `internal/trace` (`TaxonomyVersion` 1). Engine, HITL, and policy emissions use typed events (`run_started`, `tool_execution`, `hitl_request_created`, …). SQLite migration `006` adds `actor_type` and migrates legacy dot-notation types. `agentctl logs --event <type>` filters by closed vocabulary; unknown values error with the typed list. Inspector timeline and JSON payloads include `timelineIcon`, `timelineGroup`, and OTel `spanName` derived from the same enum.
