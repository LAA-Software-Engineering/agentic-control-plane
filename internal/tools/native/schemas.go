...
var operationInputSchemas = map[string]json.RawMessage{
	...
	"pull_request.create":       json.RawMessage(`{"type":"object","properties":{"owner":{"type":"string"},"repo":{"type":"string"},"head":{"type":"string","description":"Branch with the change."},"base":{"type":"string","description":"Branch to merge into (e.g. main)."},"title":{"type":"string","description":"PR title (or set issue instead)."},"issue":{"type":"integer","description":"Issue number to convert into the PR, in place of title."},"body":{"type":"string"},"draft":{"type":"boolean"}},"required":["owner","repo","head","base"]}`),
