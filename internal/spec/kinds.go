...
type WorkflowStep struct {
	...
	// Workflow invokes another Workflow resource by metadata.name as a subworkflow step
	// (issue #194, ADR 002 graph structure — the callee is statically named, not chosen by
	// an expression). Mutually exclusive with uses and agent. The step's `with` maps to the
	// callee's input; the callee's output.value becomes the step's output.
